...
export const Title = () => {
    ...
    return (
        <h1
            className={`${titleFont.className} flex flex-col items-center leading-[1.15] text-2xl sm:text-4xl lg:text-[3.2rem] font-extrabold tracking-tighter text-zinc-800`}
            style={{
        ...
        >
            {lines.map((line, lineIndex) => (
                <span
                    key={lineIndex}
                    className={`blur-word block ${lineIndex === 0 ? 'text-lg sm:text-2xl lg:text-4xl text-teal-600' : ''}`}
                >
