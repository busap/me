...
const randomFlightCode = () => {
    ...
    const maybeExtraNum =
        Math.random() < 0.25 ? Math.floor(Math.random() * 10).toString() : '';
...
const textToFlips = (
    ...
    for (let i = 0; i < maxLen; i++) {
        ...
        res.push({
            prev,
            curr,
            delayMs: baseDelay + i * stagger + Math.floor(Math.random() * 30),
            nonce: Date.now() + i,
        });
    }
    return res;
...
const FlapChar: React.FC<{ flip: CharFlip }> = ({ flip }) => {
    return (
        <span
            className="tile"
            style={{ animationDelay: `${flip.delayMs}ms` }}
            key={flip.nonce}
        >
...
export const BackgroundAirport: React.FC<{
    ...
    const recomputeRows = () => {
        ...
        newFlights.forEach((f, i) => {
            ...
            tf[i] = textToFlips(
                padTo('', TIME_SLOTS),
                padTo(f.time, TIME_SLOTS),
                base
            );
            df[i] = textToFlips(
                padTo('', DEST_SLOTS),
                padTo(f.destination.toUpperCase(), DEST_SLOTS),
                base + 100
            );
            ff[i] = textToFlips(
                padTo('', FLIGHT_SLOTS),
                padTo(f.flight.toUpperCase(), FLIGHT_SLOTS),
                base + 140
            );
        });
        setTimeFlips(tf);
            ...
            if (
                ...
                for (let k = 0; k < updates; k++) {
                    ...
                    setTimeFlips((prev) => {
                        ...
                        const prevText = padTo(
                            prev[idx]?.map((c) => c.curr).join('') || '',
                            TIME_SLOTS
                        );
                        copy[idx] = textToFlips(
                            prevText,
                            padTo(nf.time, TIME_SLOTS),
                            base
                        );
                        return copy;
                    });
                    setDestFlips((prev) => {
                        ...
                        copy[idx] = textToFlips(
                            prevText,
                            padTo(nf.destination.toUpperCase(), DEST_SLOTS),
                            base + 100
                        );
                        return copy;
                    });
                    setFlightFlips((prev) => {
                        ...
                        copy[idx] = textToFlips(
                            prevText,
                            padTo(nf.flight.toUpperCase(), FLIGHT_SLOTS),
                            base + 140
                        );
                        return copy;
                    });
